...
local ion_tof_sum=0
local ion_tof_avg=0
local ion_tof_sq_sum=0
local ion_tof_std_dev=0
local ion_tof_fwhm=0
...
function segment.other_actions() 
	...
	if ion_splat ~= 0 and ion_instance == detector_instance then
		...
		ion_tof_sq_sum=ion_tof_sq_sum+(ion_time_of_flight*ion_time_of_flight)
	...
	if ion_splat ~= 0 and ion_number == total_ions then
		...
		ion_tof_std_dev=(ion_tof_sq_sum/number_hit_detector-ion_tof_avg*ion_tof_avg)^(1/2)
		print("Ion TOF standard deviation =",ion_tof_std_dev)
		
		ion_tof_fwhm=2.354820045*ion_tof_std_dev
		print("TOF FWHM =",ion_tof_fwhm)
